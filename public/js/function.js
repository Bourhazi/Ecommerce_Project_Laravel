;(function($) {
    ...
    var MERCADO_JS = {
	    ...
	    mercado_chosen: function(){
			if($('.wrap-list-cate .test .select').length > 0){
                $('.wrap-list-cate .test .select').on('click', '.link-control', function (event) {
                    console.log('aaaaaaaaaaaa')
					event.preventDefault();
					$(this).siblings('ul').slideToggle();
                });
                $('.wrap-list-cate .select .list').on('click', 'li', function (event) {
                    var _this 	 = $(this),
						_value 	 = _this.text(),
						_content = _this.text(),
						_title 	 = _this.text();
